...
def Message processData(Message message) {
	...
	if(!pMap.get("custom_filter").isEmpty()){
	messageLog.setStringProperty("custom_filter not empty:","not empty")
	def var_filter = buildXpathQuery(message)
	
	
	messageLog.setStringProperty("var_filter:",var_filter)
	...
	//message.setBody(var_filter)
		...
		messageLog.setStringProperty("custom_filter  empty:","empty")
	}
	return message;
	}
...
def buildXpathQuery(Message message){
	...
	messageLog.setStringProperty("custom_filter length:",custom_filter_list.length.toString())
