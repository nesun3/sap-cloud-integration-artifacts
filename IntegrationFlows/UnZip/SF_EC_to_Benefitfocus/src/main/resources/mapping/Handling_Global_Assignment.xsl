		...
		<queryCompoundEmployeeResponse>
			<xsl:for-each select="queryCompoundEmployeeResponse/CompoundEmployee">
				...
				<xsl:for-each select="person">
					...
					<!-- Ignore all GA employment information nodes which are starting on a future date -->
						<xsl:choose>
							<xsl:when
								test="assignment_class='GA' and (number(translate(start_date,'-',''))) &gt;= number(translate(translate(string(current-date()),'Z',''),'-',''))">
							</xsl:when>
